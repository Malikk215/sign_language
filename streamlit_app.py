...
model_path = Path(__file__).parent / selected_model
...
with tab2:
    ...
    if st.session_state.get('model_loaded', False):
        st.info("💡 Gunakan kamera di bawah untuk mendeteksi gesture secara langsung")

        # Input kamera
        camera_image = st.camera_input("Ambil gambar gesture")
        if camera_image:
            ...
            if landmarks is not None:
                ...
                st.error("❌ Tidak ada tangan terdeteksi")
    else:
        st.warning("⚠️ Silakan load model terlebih dahulu di sidebar")
